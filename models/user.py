...
class User:
    ...
    gender: Literal["male", "female"] = field(
        default="male"
    )  # Gender as "male" or "female", default "male"
    ...
    def __post_init__(self) -> None:
        ...
        if not (MIN_AGE <= self.age <= MAX_AGE):
            raise ValueError(
                f"Age must be between {MIN_AGE} and {MAX_AGE}, got {self.age}"
            )

        # Validate weight
        if not (MIN_WEIGHT <= self.weight <= MAX_WEIGHT):
            raise ValueError(
                f"Weight must be between {MIN_WEIGHT} and {MAX_WEIGHT}, got {self.weight}"
            )
    ...
    def __hash__(self):
        return hash((self.name, self.age, self.gender, self.weight))
